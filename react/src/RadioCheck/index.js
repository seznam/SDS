 ...
 * @param {string} [props.className = ""] Space separated list of CSS classes to be added to those that RadioCheckLabel uses internaly
 * @param {number} [props.type = TYPE_CHECK] Either TYPE_RADIO for radiobutton or TYPE_CHECK for checkbox
 * @param {object} [props.forwardedRef] React ref to be forwarded to the real input with type radio or checkbox
 * @param {boolean} [props.disabled=false] Is this form field disabled?
 * @param {boolean} [props.error=false] Is the value invalid?
 * @param {string} [props.label = ""] Text to be shown next to the field as a label
 */
const RadioCheck = ({
	className = "",
	type = TYPE_CHECK,
	...
	const classes = classNames([
		"sammas-radiocheck",
		{
			"sammas-radiocheck--error": error,
			"sammas-radiocheck--disabled": disabled
		},
		className
	]);
 ...
 * @param {object} props An object with props
 * @param {string} [props.className = ""] Space separated list of CSS classes to be added to those that Checkbox uses internaly
 * @param {boolean} [props.disabled=false] Is this form field disabled?
 * @param {boolean} [props.error=false] Is the value invalid?
 * @param {string} [props.label = ""] Text to be shown next to the field as a label
 */
export const Checkbox = React.forwardRef((props, ref) => {
	return <RadioCheck {...props} forwardedRef={ref} type={TYPE_CHECK} />;
});

/**
 * Encapsulation of a radiobutton and its label
 * @param {object} props An object with props
 * @param {string} [props.className = ""] Space separated list of CSS classes to be added to those that Radio uses internaly
 * @param {boolean} [props.disabled=false] Is this form field disabled?
 * @param {boolean} [props.error=false] Is the value invalid?
 * @param {string} [props.label = ""] Text to be shown next to the field as a label
 */
export const Radio = React.forwardRef((props, ref) => {
	return <RadioCheck {...props} forwardedRef={ref} type={TYPE_RADIO} />;
});
